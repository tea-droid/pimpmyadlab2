...
- powershell -ep bypass 
 - Copy pimpmyadlab.ps1 to the vm or wget the file directly to that machine:
   ...
   `wget https://raw.githubusercontent.com/tea-droid/pimpmylatestadlab/refs/heads/main/pimpmyadlab.ps1 -O pimpmyadlab.ps1`
...
- You will be presented with a menu
 ...
 Lab build is 3 vms :
 1. Empire-DC  (Windows 2019 or 2022 Server) 
 2. Palpatine  (Windows 10 or 11 Pro or Enterprise Client)
 3. Vader (Windows 10 or 11 Pro or Enterprise Client)

 # Menu 
  - **D**  to install for Empire-DC  Domain Controller, run 1 2 and 3 
  - **P**  to install for Palpatine  Workstation #1 run 1 and 2 
  - **S**  to install for Vader Workstation #2 run 1 and 2 
...
- Domain Controller only (Windows 2019 or 2022 Server)
  ...
  - ***Run 1*** - Sets the name of the computer to Empire-DC, reboots automatically when done
...
# Workstation #1 Instructions: 
 Palpatine Workstation only (Windows 10 or 11 Pro or Enterprise Client Workstation)

  - EMPIRE-DC Domain Controller must already be completed and running to setup this workstation
  ...
  - ***Run 1*** - Sets the name of the computer to Palpatine, reboots 
    - After the reboot, Run script again, Select Menu Option P for Run #2 

  - ***Run 2*** 
    - Automatically acquires the ip address of EMPIRE-DC and assigns that ip to the DNS Configuration
    - Automatically joins the Deathstar domain
  ...
  - If the machine did not reboot automatically :
    - Is the EMPIRE-DC Domain Controller running and logged into as Administrator?
...
# Workstation #2 Instructions:
 Vader Workstation only (Windows 10 or 11 Pro or Enterprise Client Workstation)
  ...
  - ***Run 1*** 
    - Sets the name of the computer to Vader, reboots
...
# Revision 2.0.3 
  - local administrator on both vader and palpatine activated and password set to Password1 

# Revision 2.0.2 
  - local administrator account enabled on the palpatine machine and password set to Password1
...
# Revision 2.0.0
  ...
  - reworked dc, palpatine, and vader setups
...
# Revision 1.0.8 
  ...
  - function now dynamically acquires the machine name and domain-name
    instead of set to static values of "empire-dc" and "deathstar" and will now 
...
# Revision 1.0.7
  - create_deathstar_gpo function updated to add more support for the fix gpo menu option F
   - no longer statically sets "deathstar" as the domain name
...
# Revision 1.0.4 
  - Disable Defender Group Policy Object is now created with all options 
    ...
    - removes any existing "Disable Defender" Policy linked to the Deathstar.local domain
    - creates a new "Disable Defender" Policy
    - sets all settings
    - Links GPO Policy to Deathstar.local domain and Enforces policy 
      
# Revision 1.0.3 
  - Any and all Windows Updates will be removed automatically on 
    - Domain controller Empire-DC
    - Workstations : Palpatine & Vader
  ...
  Workstations Updates (Palpatine & Vader)
  - Added autoconfiguration of Palpatine to static ip instead of dhcp 
    - Palpatine  IP address will always be x.x.x.220
    - Vader IP address will always be x.x.x.221 
    - Default Gateway automatically set to x.x.x.1 for the same network ip range 
    - Subnet set to 24 masked bits 255.255.255.0
    - temporary dns of 8.8.8.8 set for installation until final configuration (run 2)
    - after run 2 of the script dns is set to EMPIRE-DC's Ip address 
...
# Revision 1.0.2 
  ...
  - Added auto join of the domain deathstar.local 
